        ...
        if len(source_row) != len(translated_row):
            issues.append(
                _structural_issue(
                    ...
                    f"{location}.row[{row_index}]: source has {len(source_row)} cells, "
                    f"translated document has {len(translated_row)}.",
                )
            )
...
def _validate_block_pair(
    ...
    if isinstance(source, ParagraphModel):
        return [
            _validate_paragraph(
                source, translated, location, source_language, target_language
            )
        ], []
...
def validate_document(
    ...
    if len(source.blocks) != len(translated.blocks):
        structural_issues.append(
            _structural_issue(
                ...
                f"Source has {len(source.blocks)} blocks, translated document has "
                f"{len(translated.blocks)}.",
            )
        )
    ...
    status = ValidationStatus.PASS
    if any(issue.severity is ValidationStatus.FAILURE for issue in structural_issues):
        status = ValidationStatus.FAILURE
    elif any(item.report.status is ValidationStatus.FAILURE for item in items):
        status = ValidationStatus.FAILURE
    elif structural_issues or any(
        item.report.status is ValidationStatus.WARNING for item in items
    ):
        status = ValidationStatus.WARNING
